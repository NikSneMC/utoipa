...
pub struct ScalarConfig {
    ...
    /// Controls when developer tools are shown.
    pub show_developer_tools: Option<ScalarShowDeveloperTools>,

    /// Configuration for Scalar MCP (Model Context Protocol) integration.
    pub mcp: Option<ScalarMcp>,}

/// Configuration for Scalar MCP (Model Context Protocol) integration.
///
/// Enables or disables MCP and provides connection details if not disabled.
#[derive(Clone, Debug)]
pub enum ScalarMcp {
...
impl Serialize for ScalarMcp {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer
    {
        let mut s = serializer.serialize_struct(stringify!(ScalarMcp), 3)?;

        let (name, url, disabled) = match self {
            Self::Enabled { name, url } => (name.as_str(), url.as_str(), false),
            Self::Disabled => ("", "", true)
        };

        s.serialize_field("name", name)?;
        s.serialize_field("url", url)?;
        s.serialize_field("disabled", &disabled)?;
        s.end()
    }
}
...
#[serde(rename_all = "camelCase")]
pub enum ScalarShowDeveloperTools {
    /// Always show developer tools.
    Always,
    /// Show only on localhost and domains often used for development.
    Localhost,
    /// Never show.
    Never,
}
